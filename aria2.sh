...
check_new_ver(){
	echo -e "${Info} 请输入 Aria2 版本号，格式如：[ 1.34.0 ]，获取地址：[ https://hub.fastgit.org/q3aql/aria2-static-builds/releases ]"
	...
	if [[ -z ${aria2_new_ver} ]]; then
		...
		if [[ -z ${aria2_new_ver} ]]; then
			echo -e "${Error} Aria2 最新版本获取失败，请手动获取最新版本号[ https://hub.fastgit.org/q3aql/aria2-static-builds/releases ]"
...
Download_aria2(){
	...
	wget -N --no-check-certificate "https://hub.fastgit.org/q3aql/aria2-static-builds/releases/download/v${aria2_new_ver}/aria2-${aria2_new_ver}-linux-gnu-${bit}-build1.tar.bz2"
...
Download_aria2_conf(){
	mkdir "${file}" && cd "${file}"
	wget --no-check-certificate -N "https://raw.fastgit.org/ToyoDAdoubiBackup/doubi/master/other/Aria2/aria2.conf"
	[[ ! -s "aria2.conf" ]] && echo -e "${Error} Aria2 配置文件下载失败 !" && rm -rf "${file}" && exit 1
	wget --no-check-certificate -N "https://raw.fastgit.org/ToyoDAdoubiBackup/doubi/master/other/Aria2/dht.dat"
...
Service_aria2(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate https://raw.fastgit.org/ToyoDAdoubiBackup/doubi/master/service/aria2_centos -O /etc/init.d/aria2; then
		...
		if ! wget --no-check-certificate https://raw.fastgit.org/ToyoDAdoubiBackup/doubi/master/service/aria2_debian -O /etc/init.d/aria2; then
			echo -e "${Error} Aria2服务 管理脚本下载失败 !" && exit 1
		fi
		chmod +x /etc/init.d/aria2
...
Update_bt_tracker_cron(){
	...
	check_pid
	[[ ! -z ${PID} ]] && /etc/init.d/aria2 stop
	bt_tracker_list=$(wget -qO- https://raw.fastgit.org/ngosang/trackerslist/master/trackers_all.txt |awk NF|sed ":a;N;s/\n/,/g;ta")
...
Update_Shell(){
	sh_new_ver=$(wget --no-check-certificate -qO- -t1 -T3 "https://raw.fastgit.org/ToyoDAdoubiBackup/doubi/master/aria2.sh"|grep 'sh_ver="'|awk -F "=" '{print $NF}'|sed 's/\"//g'|head -1) && sh_new_type="github"
	[[ -z ${sh_new_ver} ]] && echo -e "${Error} 无法链接到 Github !" && exit 0
	if [[ -e "/etc/init.d/aria2" ]]; then
		rm -rf /etc/init.d/aria2
		Service_aria2
	fi
	wget -N --no-check-certificate "https://raw.fastgit.org/ToyoDAdoubiBackup/doubi/master/aria2.sh" && chmod +x aria2.sh
